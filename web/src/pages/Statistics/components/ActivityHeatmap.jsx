    ...
    <div ref={containerRef}>
      ...
      <div className='overflow-x-auto'>
        ...
        >
          ...
          {data.cells.map((cell, i) => {
            ...
            const level = Math.min(cell.count, LEVEL_FILL.length - 1);
            ...
            return (
              <rect
                ...
                height={cellSize}
                rx={2}
                className={isFuture ? FUTURE_FILL : LEVEL_FILL[level]}
      ...
      <div className='mt-3 flex flex-wrap items-center gap-x-5 gap-y-1 text-xs'>
        ...
        <span className='opacity-60'>
          Longest streak:{' '}
          <span className='text-base-content font-bold'>
            {data.stats.longestStreak} {pluralize(data.stats.longestStreak, 'day', 'days')}
          </span>
        </span>
        <span className='opacity-60'>
          Current streak:{' '}
          <span className='text-base-content font-bold'>
            {data.stats.currentStreak} {pluralize(data.stats.currentStreak, 'day', 'days')}
          </span>
        </span>
      </div>
      ...
      {tooltip && (
        ...
        >
          <div className='font-semibold'>
            {tooltip.cell.count} {pluralize(tooltip.cell.count, 'shot', 'shots')}
          </div>
          <div className='opacity-60'>{formatDateLocal(tooltip.cell.dayMs, TOOLTIP_DATE_OPTIONS)}</div>
        </div>
      )}
    </div>
  );
